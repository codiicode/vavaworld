...
/** An arbitrary HTML overlay pinned to a lat/lng (rotates with the globe and
 *  hides when it's on the far side). Used for city labels + live buy cards. */
export interface GlobePopup {
  id: string;
  lat: number;
  lng: number;
...
interface Globe3DProps {
  ...
  markers?: GlobeMarker[];
  /** Array of HTML popups pinned to lat/lng (city labels, live buy cards) */
  popups?: GlobePopup[];
  /** Globe configuration */
...
// ============================================================================
// Popup Component - arbitrary HTML pinned to a lat/lng (city labels, buy cards)
// ============================================================================

function Popup({ popup, radius }: { popup: GlobePopup; radius: number }) {
  const ref = useRef<THREE.Group>(null);
  const { camera } = useThree();
  const [visible, setVisible] = useState(true);

  const pos = useMemo(
    () => latLngToVector3(popup.lat, popup.lng, radius * 1.02),
    [popup.lat, popup.lng, radius],
  );

  // Hide when the point rotates to the far side of the globe.
  useFrame(() => {
  ...
  return (
    <group ref={ref} position={pos}>
      <mesh>
        <sphereGeometry args={[popup.dotSize ?? 0.016, 16, 16]} />
        <meshBasicMaterial color={popup.dotColor ?? "#2dd4bf"} />
      </mesh>
      {popup.node && (
        <Html center style={{ pointerEvents: "none", opacity: visible ? 1 : 0, transition: "opacity 0.18s ease-out" }}>
          {popup.node}
        </Html>
      )}
    </group>
...
function RotatingGlobe({
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* Popups - city labels + live buy cards, pinned to lat/lng */}
      {popups.map((popup) => (
        <Popup key={popup.id} popup={popup} radius={config.radius} />
      ))}
    </group>
  );
}
...
function Scene({ markers, popups, config, onMarkerClick, onMarkerHover }: SceneProps) {
  const { camera } = useThree();
